...
We have one more join to do before we can create the plot. The `data_full` data frame needs to be joined to the `GeorgiaCounty` data frame we created earlier that contains the counties and dates we want to plot. Before we can join, look at the `county` column in `data_full` and `subregion` column in `GeorgiaCounty`. These are both referring to the counties in Georgia, but the `county` entries are capitalized while `subregion` entries are not. If we want to join these data frames, the quickest way will be to make the `county` column entries lowercase using `str_to_lower` and then joining.
...
georgia_map <- georgia_map %>% 
  mutate(manual_fill = cut(rate, breaks = c(-Inf, 0, 621, 1071, 1623, 2960, Inf),
                           labels = c("None", "1-620", "621-1,070", "1,071-1,622", "1,623-2,960", ">2,960"),
                           right = TRUE))
