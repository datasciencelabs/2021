...
### Example
Let's look at a concrete example. This example is based on the example from [Mastering Shiny](https://mastering-shiny.org/basic-reactivity.html) (chapter 3) and the live app can be accessed [here](https://hadley.shinyapps.io/basic-reactivity-cs/).
...
Note: to type $\mu$ on a Mac keyboard: `option + m`
...
A better look at what we have done with reactive expressions is to look at the graph in a modular form:

<center><img src="images/modules.png" style="width: 220px;"/></center>

### Setting a timer

Suppose we want to reinforce the fact that this is for simulated data by constantly resimulating the data, so that you see an animation rather than a static. We can increase the frequency of updates with a new function: `reactiveTimer()`.

`reactiveTimer()` is a reactive expression that has a dependency on a hidden input: the current time. You can use a `reactiveTimer()` when you want a reactive expression to invalidate itself (update) more often than it otherwise would. For example, the following code uses an interval of 500 ms so that the plot will update twice a second. This is fast enough to remind you that you’re looking at a simulation, without dizzying you with rapid changes.

```{r, eval=FALSE}
server <- function(input, output, session) {
    # Add timer. Here, 500 means update every 500 ms (twice every minute)
...
ui <- fluidPage(
    ...
    fluidRow(
        column(4, actionButton("simulate", "Simulate!"))
    ),
...
We can see the `Simulate!` click button now. 

<center><img src="images/simulate_button.png" style="width: 700px;"/></center>

However, this doesn’t achieve our goal because it just introduces a new dependency: `x1()` and `x2()` will update when we click the simulate button, but they’ll also continue to update when `n1`, `n2`, `mean1`, `mean2`, `sd1`, or `sd2` change. We want to replace the existing dependencies, not add to them.
...
```{r, eval=FALSE}
server <- function(input, output, session) {
    # Define x1 and x2 as event reactive expressions
...
Basically, the plots won't update even when `n1`, `n2`, `mean1`, `mean2`, `sd1`, or `sd2` change until the `Simulate!` button is pushed. This saves computing time. 
