...
Here I'm using the `vroom` package and function to read the data in faster since the datasets we'll be using for this example are much bigger than the ones we have been using thus far in the course. The double colon (`::`) means `package::functionname`. Some packages in R have the same name for functions included in the package, but the functions may do different things. If you want to use a function from a specific package that has the same name as a function in a different package, it's best to use the double colon and specify both the package you have in mind and the function you want to use. This is particularly important when using the `dplyr` and `tidyverse` packages. They both contain functions with the same names. 
...
summary %>% ggplot(aes(age, n, colour = sex)) + 
            geom_line() + 
            labs(y = "Estimated number of injuries", x = "Age")
```
...
Now that we have the basic components in place, and working, we can progressively improve our app. The first problem with this app is that it shows a lot of information in the tables, where we probably just want the highlights. To fix this we need to need to first figure out how to truncate the tables. I’ve chosen to do that with the `fct_lump_n` function: I convert the variable to a factor, order by the frequency of the levels, and then lump together all levels after the top 5. The number 5 is arbitrary - I chose it because I think it looks nice. 

```{r}
injuries %>%
  mutate(diag = fct_lump_n(diag, 5)) %>%
  group_by(diag) %>%
  summarise(n = as.integer(sum(weight)))
```

Now I can write a function to automate this for any variable. Here, `{{ var }}` "embraces" any variable between the double curly brackets and uses that variable as the column input for the mutate function.
...
I then add `width = "100%"` to the `renderTable` function in the server function to make each column span the entire space I've allowed it, making each column uniform in width. 
...
I made one other change to improve the aesthetics of the app: I forced all tables to take up the maximum width (i.e. fill the column that they appear in). This makes the output more aesthetically pleasing because it reduces the amount of extraneous variation.
...
Let's put it all together here once again using the `shinyApp()` function.
