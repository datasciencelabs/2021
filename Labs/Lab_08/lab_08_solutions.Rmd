---
title: 'Lab 8: Machine Learning III'
output:
  html_document
---
...
## 3. Midterm Review and Practice Problems
...
* All questions with answers from the teaching staff will be posted to a google doc (link will be shared) so everyone has access to the same information. It is your responsibility to check this document. 

* Your repository should include a knittable .Rmd file, and corresponding .html file. 
...
* No late days are available for this exam unless you have documented accommodations and have been in touch with Colleen Cronin in the Student Affairs Office.
...
Probability **will not** be on the exam. Machine learning algorithms including QDA, LDA, decision tress, and Random Forest **will not** be on the exam. 

A midterm repository in GitHub will be set up for you with the Rmd, html and data files included (just like a homework assignment). 
...
**Answer: `log10` since `population` is so large**
...
**Answer: `select()`. `select()` is for columns and `filter()` is for rows**

#### Question 4

Load the following data set with player salaries:
```{r, message=FALSE}
salaries <- read_csv("https://raw.githubusercontent.com/datasciencelabs/data/master/Salaries.csv")
```

True or False: this table has exactly one row for each player.

**Answer: False**

#### Question 5

True or False: the output of the following code is a data frame.
```{r}
...
**Answer: False. The `.$` makes this a number.**

#### Question 6
If I want to make a plot showing the relationship between 2 continuous variables, which ggplot2 geometry should I use?
...
Read a description of the data set [here.](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/mtcars.html)

What values are contained in the am column?

* Transmission (0 = automatic, 1 = manual)
* Auto Mechanic
* There is no `am` column
* Number of cylinders
...
(Don't actually need to load the data, just read the documentation)

**Answer: Transmission (0 = automatic, 1 = manual)**

#### Question 8

Run the following code to read in the following data set and split `Date` into year, month and day:
```{r, message=FALSE}
require(tidyverse)
ny_airquality <- read_csv("https://raw.githubusercontent.com/datasciencelabs/data/master/ny_airquality.csv")

ny_airquality <- ny_airquality %>% separate(Date, c("Year", "Month", "Day"), sep = "-")
```

Then use the `group_by` and `summarize` functions to compute the average temperature for each month. How many months have average temperatures above 80 degrees?

```{r}
df <- ny_airquality %>% group_by(Month) %>%
...
For this question we will use the following 2 tables:
```{r, message=FALSE}
library(readr)
...
player_info <- master %>% 
  dplyr::select(playerID, nameFirst, nameLast, birthYear, height)

salaries <- read_csv("https://raw.githubusercontent.com/datasciencelabs/data/master/Salaries.csv")
```

Create a table with one row for each player that shows their average salary. Use one of the dplyr join functions to add average salaries to the `player_info` table. Of the players born after 1986, who had the highest average salary?

```{r}
avg_salary <- salaries %>% group_by(playerID) %>%
  summarize(avg_salary = mean(salary))

df <- left_join(player_info, avg_salary, by = "playerID") %>%
...
Using the `babynames` dataset, how many girl names contain the string `mira` or `Mira`?
