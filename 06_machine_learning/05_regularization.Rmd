...
[the winners were announced](https://archive.nytimes.com/www.nytimes.com/2009/09/22/technology/internet/22netflix.html). You can read a good summary of how the winning algorithm was put together [here](http://blog.echen.me/2011/10/24/winning-the-netflix-prize-a-summary/).
...
We start by loading a [large, compressed movie ratings dataset](https://github.com/datasciencelabs/data/blob/master/movielens-train.csv.gz) from IMDB into R. 

```{r}
ratings <- read_csv("movielens-train.csv.gz")
head(ratings)
```
...
`userId`: unique identifier given to individual rating the movie

`movieId`: unique identifier given to the movie being rated

`rating`: movie rating given by user ranging from 1 (low) to 5 (high)

`timestamp`: unique identifier for time rating was submitted 

Each row represents the rating of a particular movie by a particular user. 
...
We can also see the distribution of all ratings. Here we take a random sample of the ratings so we can easily and quickly plot the distribution. 

```{r}
ratings %>% 
  sample_n(10000) %>% # Take sample to easily make plot
  ggplot(aes(rating)) + 
  geom_bar(color = "black")
```

Another dataset available to us is the `movies` dataset. Each row corresponds to a movie and the columns available are

`movieID`: unique identifier given to the movie (identical to the movie Ids in the `ratings` dataset)

`title`: movie title

`genres`: genre(s) of the movie (ex: Romance, Comedy, Children, etc.)

It will be helpful for analysis to create a new data frame containing information from both the `ratings` and `movies` datasets. We can think of this data frame as a very large matrix with users on the rows and movies on the columns. The `spread` function permits us to convert it to this format, but if we try it for the entire matrix it will crash R. Let's instead show the matrix for a few users. Note that the movie titles in the `movies` data frame contain the year the movie was released and we remove this extra information to save space and memory.
...
movies$title <- str_replace(movies$title, "\\s\\([^)]*\\)", "") # Take out (year) from movie titles
...
Now that we know some basic facts about our dataset, let's randomly split the data into training and test sets. 
...
# Save memory by removing (rm) the ratings data frame
rm(ratings)
...
Note that there are movies in the test set that are not on the train set. This means we don't have predictions and the `join` above turns these into NAs. Since we have no data, we will simply predict with the average $b_i$ which is 0.
...
Let's explore where we made mistakes. Here we calculate the difference between our predicted rating and the truth and display in descending order so we can see which movies we are the worst at predicting. Note we show the absolute value because we are interested in the magnitude of the difference, and not the sign. In other words, a value of 4.5 and -4.5 have the same magnitude and we are equally bad at predicting a rating for these movies. 
...
$$\frac{1}{M \times N} \sum_{u=1}^M \left(Y_{u,i} - \mu - b_i\right)^2 + \lambda \sum_{i=1}^N b_i^2$$

where $M$ is the total number of users and $N$ is the total number of movies. Using calculus we can actually show that the values of $b_i$ that minimize this equation are:
...
So it looks like a value of $\lambda = 2.25$ gives us the smallest RMSE.
...
$$
where $n_u$ is the number of users who have rated movie $i$.
...
Another common strategy for ratings prediction is matrix factorization. **This approach is very much related to principal component analysis (PCA). This problem will demonstrate how to use PCA to uncover broad, latent patterns in user/movie relationships, and how to use the results of PCA to predict unknown user/movie relationships**.

To use the tools we have learned we need to create a manageable dataset. We will do this by filtering the training set to movies that have been widely rated (rated by many users). We will also focus on users that rate many movies. Finally, we focus on movies and users that appear in the test set.
...
j <- grep("Goodfellas", movie_titles)
...
with $\mathbf{p} = (p_1, \dots, p_i)$ a $12\times 1$ vector and $\mathbf{q}=(q_1, \dots, q_5)$ a $5 \times 1$ vector. We started with 60 residuals but it appears we can explain most of the variability with just a few parameters. 
