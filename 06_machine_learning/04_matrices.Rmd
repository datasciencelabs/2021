...
The _dimension_ of a matrix is often an important characteristic needed to assure certain operations can be performed. The _dimension_ is a two number summary defined as the number of rows $\times$ the number of columns. In R we can extract the dimension of a matrix with the function `dim`:
...
We will learn several useful operations related to matrix algebra. We use a few of the motivating examples listed above.
...
```{r}
mat_t <- matrix(my_vector, 5, 3, byrow = TRUE)
mat_t
```
...
We notice a clear dichotomy which is explained as parts with ink and parts without. If we think that values below, say, 25 are smudges we can quickly make them zero using:
